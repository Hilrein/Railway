...
def promokod(request):
    return render(request, 'promokod.html')
...
def sayahat(request):
    return render(request, 'sayahat.html')
            ...
            for route in routes:
                ...
                if from_city in stations and to_city in stations:
                    ...
                    if from_city_index < to_city_index:  
                        ...
                        matching_trains.extend([schedule.train for schedule in schedules])  

            return render(request, 'list.html', {'trains': matching_trains})  
        else:
...
def payment_success(request):
    if request.method == 'POST':
        ...
        ticket_text = f'Сіздің билетіңіз №{latest.id}!\n\nПоезд №{train_id}.\nБағыт: {train.route}.\nВагон: {wagon_id}.\nҚызмет көрсету классы: {wagon_type}.\nОрын: {seat}.\nБилет бағасы: {ticket_price}₸.\n\nБізбен болғаныңыз үшін рахмет!'
...
def send_email(text, title, *emails):
    email_sender = "sapartime@inbox.ru"
    email_getters = emails

    smtp_server = SMTP("smtp.mail.ru", 587)
    smtp_server.starttls()

    smtp_server.login(email_sender, '6dYnanYp3dgutba9HyyA')
